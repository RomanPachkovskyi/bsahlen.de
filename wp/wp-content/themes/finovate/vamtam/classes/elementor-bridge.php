...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'finovate' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'finovate' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'finovate' ),
				'h1'           => __( 'H1', 'finovate' ),
				'h2'           => __( 'H2', 'finovate' ),
				'h3'           => __( 'H3', 'finovate' ),
				'h4'           => __( 'H4', 'finovate' ),
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'finovate' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'finovate' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'nested-accordion'  => [ 'label' => __( 'Accordion', 'finovate' ) ],
			'button'            => [ 'label' => __( 'Button', 'finovate' ) ],
			'icon'              => [ 'label' => __( 'Icon', 'finovate' ) ],
			'icon-list'         => [ 'label' => __( 'Icon List', 'finovate' ) ],
			'container'         => [ 'label' => __( 'Container', 'finovate' ) ],
			'loop-grid'         => [ 'label' => __( 'Loop Grid', 'finovate' ) ],
			'nav-menu'  	    => [ 'label' => __( 'Nav Menu', 'finovate' ) ],
			'nested-tabs'       => [ 'label' => __( 'Tabs', 'finovate' ) ],
			'form'              => [ 'label' => __( 'Form', 'finovate' ) ],
			'toggle'            => [ 'label' => __( 'Toggle', 'finovate' ) ],
			'post-comments'     => [ 'label' => __( 'Post Comments', 'finovate' ) ],
			'search'            => [ 'label' => __( 'Search', 'finovate' ) ],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'finovate' ) );
		}
	...
	public static function print_registered_widgets_assets_data() {
		...
		if ( in_array( 'forms', $registered_widgets ) || isset( $widgets_data['forms'] ) ) {
			...
			if ( $inline_font_icons_active ) {
				// Forms need eicons & fa library cause they contain icons in CSS pseudo-element format.
