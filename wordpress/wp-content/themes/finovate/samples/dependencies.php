...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Elementor', 'finovate' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html( sprintf( __( 'Vamtam Elementor Integration (%s)', 'finovate' ), VAMTAM_THEME_SLUG ) ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'finovate' ),
		...
		array(
			'name'     => esc_html__( 'Ally - Web Accessibility & Usability', 'finovate' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'finovate' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'finovate' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'finovate' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'finovate' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'WP Super Cache', 'finovate' ),
			'slug'     => 'wp-super-cache',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'finovate' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'finovate' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'finovate' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'finovate' ),
		'description' => esc_html__( 'Description', 'finovate' ),
		'status'      => esc_html__( 'Status', 'finovate' ),
		'version'     => esc_html__( 'Version', 'finovate' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'finovate' );
	}
	if ( strpos( $plugin['slug'], 'finovate' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'finovate' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'finovate' );
