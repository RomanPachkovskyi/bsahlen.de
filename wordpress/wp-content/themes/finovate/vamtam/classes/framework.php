...
class VamtamFramework {
	...
	public static function theme_supports() {
		...
		function vamtam_elementor_widgets_support( bool $supports, array $args, string $feature ) {
			...
			$theme_features = [
				...
				'nav-menu--contain-link-to-text',
				'button--use-theme-icon-styles',
				'button--content-align-fix',
				'form--btn-hover-anim',
				'nested-accordion--use-icon-theme-styles',
				'icon-list--use-icon-theme-styles',
				'container--clip-path-section',
				'loop-grid--load-more-btn-hover-anim',
				'nested-tabs--use-theme-style',
			];
		...
		if ( function_exists( 'register_nav_menus' ) ) {
			register_nav_menus(
				array(
					'primary-menu'     => esc_html__( 'Menu Header', 'finovate' ),
				)
			);
		}
	}
	...
	public static function load_languages() {
		load_theme_textdomain( 'finovate', VAMTAM_THEME_DIR . 'languages' );
	}
	...
	public static function license_valid() {
		...
		esc_html_e( 'Valid Purchase Key.', 'finovate' );
		echo '<p>';
		esc_html_e( 'Congratulations! You have succesfully registered your product.', 'finovate' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_invalid() {
		echo '<span id="fail">';
		esc_html_e( 'Incorrect Purchase Key.', 'finovate' );
		echo '<p>';
		esc_html_e( 'Please check your purchase code and re-enter.', 'finovate' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_failed() {
		...
		esc_html_e( 'Cannot validate Purchase Key. Please try again later. If the problem persists your server might not have the curl PHP extension enabled.', 'finovate' );
		echo '</span>';
	}
				...
				<div class="vamtam-notice vamtam-license-notice notice">
					...
					<div class="vamtam-notice-content">
						...
						<?php if ( $is_token ) : ?>
							...
							<?php
								echo wp_kses( sprintf( __( ' <a href="%s" target="_blank"> this link</a>', 'finovate' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
	...
	public static function token_unregistered() {
		echo '<span id="success">';
		esc_html_e('Unregistered Token.', 'finovate');
		echo '</span>';
	}

	public static function token_unregister_fail() {
		echo '<span id="fail">';
		esc_html_e('Failed to unregister token. Please try again later. If this issue persists please contact support.', 'finovate');
		echo '</span>';
	}

	public static function license_unregister() {
		echo '<span id="success">';
		esc_html_e( 'Unregistered Key.', 'finovate' );
		echo '</span>';
	}
